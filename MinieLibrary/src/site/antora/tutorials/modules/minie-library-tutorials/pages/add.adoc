...
TIP: If you're creating a JMonkeyEngine desktop app from scratch,
...
. Create, configure, and attach
  ...
  if the app doesn't already do so.
...
If any directory where the app might run contains files
...
.. Select the "Minie-9.0.3.jar" file.
...
.. Select the "Minie-9.0.3-javadoc.jar" file.
.. Click on the btn:[Open] button.
.. Click on the btn:[Browse...] button to the right of "Sources:"
.. Select the "Minie-9.0.3-sources.jar" file.
...
In a conventional JMonkeyEngine app,
the required native libraries get loaded automatically by `Application.start()`.

If your app doesn't invoke `Application.start()`,
...
the app can access immediately:
...
* Before adding Minie to an app, remove all other physics libraries.

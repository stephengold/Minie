...
Within a ready ragdoll, each link can be independently configured
as dynamic (controlled by physics) or kinematic (not affected by physics).
...
* `Bound`, which sets bone rotations to their bind positions,
...
its possible for a ragdoll to be part dynamic, part kinematic.

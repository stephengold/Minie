...
:url-examples-shapes: https://github.com/stephengold/Minie/tree/master/MinieExamples/src/main/java/com/github/stephengold/shapes
...
{url-tutorial}/HelloMadMallet.java[HelloMadMallet] is a simple app
...
Some apps require collision shapes that are _scalable_
...
CollisionShape shape
        = CollisionShapeFactory.createVhacdShape(modelRoot, v4Parms, null);
...
The V-HACD algorithm is costly to run, but for many apps it can

...
While most collision-shape subtypes are precise, 5 are not.
...
* {url-shape}/HullCollisionShape.html[`HullCollisionShape`],
* {url-shape}/MultiSphere.html[`MultiSphere`], and
* {url-shape}/SimplexCollisionShape.html[`SimplexCollisionShape`]
  with 4 vertices.
...
. After landing, the mallet comes to rest on its side,
  which is the most natural behavior for a mallet.
...
Its `createDynamicMeshShape()` method

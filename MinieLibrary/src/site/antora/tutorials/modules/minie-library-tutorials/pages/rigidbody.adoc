...
because the events described do not, by themselves, reactivate it.
...
The simulator sees
that the dynamic body doesn't intersect the obstable on any time step,
...
this solution can become very inefficient.
...
and performs a more accurate collision test on any obstacles it finds.

Because CCD involves extra computation, it is disabled by default.
Because it is only necessary for fast-moving bodies, it is enabled only
when a body's distance travelled per time step exceeds a threshold.
To enable CCD for a particular rigid body,
increase the body's CCD activation threshold using
`rigidBody.setCcdMotionThreshold(distancePerTimeStep)`.

To get the best possible results using CCD,
configure both the motion threshold and the size of the swept sphere.

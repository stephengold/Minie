...
* A "+big3" build, built using the "-Pbig3" option,
  with support for just the 3 main platforms (Linux64, MacOSX64, and Windows64).
* A "+bare" build, built using the "-Pbare" option,
  which doesn't include *any* native libraries.
* A "+ano" build, built using the "-Pano" option,
  which includes native libraries for Android only and *no* Java classes.
...
NOTE:
Native libraries aren't available for every possible combination of options.
For instance, double-precisions native libraries aren't available for Android platforms.
For further information, see the Libbulletjme project.
